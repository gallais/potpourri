...
for \IdrisFunction{poke} (note that it looks eerily
...
The \IdrisType{Out} family describes formally what revealing the
top node means:
...
%
Operationally, remembering the data layout detailed in
\Cref{fig:data-layout}, the implementation of \IdrisFunction{out}
will amount to inspecting the tag used by the node and
then deserialising the offsets stored immediately after it.
...
(defined later) to put together the pointer to the meaning of
the description associated to the index \IdrisBound{k}.
