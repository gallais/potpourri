...
itself;
\AIC{keep} extend an existing injection into one that preserves
...
Staging this definition does evaluate away all of the functions
and function calls to yield a simple circuit obtained by sequentially
...
static layer, we can once again define more interesting circuits like the
following \AF{`tab} term which takes a function that takes a boolean
and computes a one-input one-output and returns a circuit with two outputs
and one output that has the same behaviour.

\ExecuteMetaData[MetaCircuit.tex]{tab}

It is natural to want to push this even further and generalise that
term to one tabulating functions whose computed circuit has an
arbitrary number of inputs and outputs.
%
This would be possible in a setting where the static layer is dependently
typed like in Kov{\'{a}}cs' original paper.
