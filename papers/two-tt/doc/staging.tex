...
Quipper, a functional programming language to describe
quantum computations introduced by Green, Lumsdaine, Ross, Selinger,
and Valiron~\cite{DBLP:conf/rc/GreenLRSV13}, gives us a
good example of a setting in which we can have two very distinct
layers.

Indeed, by defining a small functional hardware description language
we can investigate a case study where functions are only available
in the static layer, with only circuits in the dynamic one.
In our experiment, we use a minimal circuit language inspired by
Flor, Swierstra, and Sijsling's work on formal hardware description
and verification~\cite{DBLP:conf/types/FlorSS15}.
