...
(GADT) fashion.
First, \AIC{`ℕ} stands for the natural numbers acting as our base type.
And second, (\AB{A} \AIC{`⇒} \AB{B}) is the type of functions from
\AB{A} to \AB{B}.
...
\todo{Replace \AD{Bool} with \AD{Phase}}
...
that the status and stage indices are polymorphic here.
...
\ExecuteMetaData[BasicTwoTT.tex]{termaliases}
...
Static values of type \AIC{`α} do not exist as this base type does
not have any associated constructors;
