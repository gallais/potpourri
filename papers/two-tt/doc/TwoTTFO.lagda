...
data Term : Phase → ∀ st → Type st → Context → Set where
  ...
  `app   : ∀[ Term source _ (A `⇒ B) ⇒ Term source _ A ⇒ Term source _ B ]
  `lam   : ∀[ (_, A) ⊢ Term source _ B ⇒ Term source _ (A `⇒ B) ]
\end{code}
%<*termnat>
\begin{code}
  `zero  : ∀[ Term ph st `ℕ ]
  `succ  : ∀[ Term ph st `ℕ ⇒ Term ph st `ℕ ]
  `iter  : ∀[ Term ph _ (`ℕ `⇒ (A `⇒ A) `⇒ A `⇒ A) ]
\end{code}
%</termnat>
\begin{code}
...
infixr 3 _`∘_
_`∘_ : ∀[ Term source _ (B `⇒ C) ⇒ Term source _ (A `⇒ B) ⇒ Term source _ (A `⇒ C) ]
...
iter : ∀ {A} → Value _ (`ℕ `⇒ (A `⇒ A) `⇒ (A `⇒ A)) Γ
-- iter dynamic  = `iter
iter    = λλ[ _ , m ] λλ[ _ , succ ] λλ[ σ , zero ]
...
%<*app>
\begin{code}
app : ∀  {A B} → Value static (A `⇒ B) Γ → Value static A Γ → Value static B Γ
app   = _$$_
\end{code}
%</app>
\begin{code}
...
\begin{code}
lam : {A B : Type _} →
      Kripke (Value _ A) (Value _ B) Γ →
      Value _ (A `⇒ B) Γ
lam    b = λλ[ σ , v ] b .runBox σ v
...
eval ρ (`app {A = A} {B} f t)  = app {A = A} {B = B} (eval ρ f) (eval ρ t)
eval ρ (`lam {A = A} {B = B} b)    = lam {A = A} {B = B} (body ρ b)
eval ρ (`zero {st = st})     = zero st
eval ρ (`succ {st = st} n)   = succ st (eval ρ n)
eval ρ (`iter {A = A})     = iter {A = A}
eval ρ `⟨ t ⟩                = eval ρ t
eval ρ (`∼ v)                = eval ρ v
