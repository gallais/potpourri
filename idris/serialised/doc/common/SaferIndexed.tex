...
%<*pokefunProd>
\begin{code}
    poke {d = Prod {sl, ol} d e} {t} el = do
      let (subl, subr) = splitAt ol (subterms el)
      let left = MkMeaning subl (meaningBuffer el) (meaningPosition el)
      let pos = meaningPosition el + sum subl + cast sl
      let right = MkMeaning subr (meaningBuffer el) pos
      pure (rewrite etaTuple t in left # right)
\end{code}
%</pokefunProd>
%<*pokefunRec>
\begin{code}
    poke {d = Rec} el = pure (MkMu (meaningBuffer el) (meaningPosition el))
\end{code}
%</pokefunRec>
%</pokefunbody>
\begin{code}
...
\begin{code}
    ...
    getConstructor (MkIndex k) mu
      = getOffsets
        (muBuffer mu)
        (1 + muPosition mu)
        (offsets (index k $ constructors cs))
