...
%<*offer>
\begin{code}
offer : LinearIO io => {s1, s2 : _} ->
  (1 ch : Channel (Offer s1 s2) e) ->
  L1 io (Res Bool $ \ b => ifThenElse b (Channel s1 e) (Channel s2 e))
\end{code}
%</offer>
\begin{code}
...
%<*select>
\begin{code}
select : LinearIO io => {s1, s2 : _} ->
  (1 ch : Channel (Select s1 s2) e) ->
  (b : Bool) -> L1 io (ifThenElse b (Channel s1 e) (Channel s2 e))
\end{code}
%</select>
\begin{code}
...
unfold : LinearIO io =>
  {pos : _} ->
  Channel (Rec pos) e -@
  L1 io (Channel (head (lookup pos e)) (lookup pos e))
...
fold : LinearIO io =>
  {pos : _} ->
  Channel (Rec pos) e -@
  L1 io (Channel (Fix nm (head (lookup pos e))) (tail (lookup pos e)))
...
main = do
  ...
  (_, sums) <- LIO.run $ fork Sumer (natssSum 0) (sumNatss [[1..5], [5..10]] [<])
