...
The prototypical examples are probably the types of vectors (\IdrisType{Vect})
...
\IdrisData{0}, and its \IdrisData{(::)} constructor combines a
...
%
Its \IdrisData{Z} constructor models to \IdrisData{0} and is bound by any
...
This leads us to the definition of the \IdrisFunction{lookup} function. Provided
a vector of size \IdrisBound{n} and a finite number bound by this same \IdrisBound{n},
...
matching clauses as the return type of all of the \IdrisType{Fin} constructor forces
...
\IdrisFunction{lookup} can be compiled to a function taking two natural number and a list.
...
Values may be runtime irrelevant (\IdrisKeyword{0} quantity),
linear (\IdrisKeyword{1} quantity) or unrestricted (denoted by no quantity annotation).
We will not use linearity in this study.
...
packs two numbers and a runtime irrelevant proof. We have
%
a natural number called \IdrisFunction{bigEnd} corresponding to the size of the
big end of the thinning (\IdrisBound{sy})
%
an integer called \IdrisFunction{encoding} corresponding to the thinning
represented as a bit vector. We only care about the \IdrisFunction{bigEnd} first
bits and assume the rest is 0.
%
and a runtime irrelevant proof \IdrisFunction{thinning} that \IdrisFunction{encoding}
is indeed a valid encoding of size \IdrisFunction{bigEnd} of a thinning from
\IdrisBound{sx} to \IdrisBound{sy}.

\ExecuteMetaData[Thin.idr.tex]{thin}

We can define functions acting as smart constructor analogues to the inductive
family's data constructors.
%
The \IdrisFunction{done} function packs a pair of \IdrisData{0} (the size of the
big end, and the encoding) together with a proof that it is an adequate encoding
of the thinning from the empty scope to itself.
...
thinning getting rid of the variable \IdrisBound{x} freshly added to the source
scope.

\ExecuteMetaData[Thin.idr.tex]{drop}

These smart constructors allow us to seamlessly build thinnings. To recover the
full expressive power of the inductive family, we also need to be able to take
these thinnings apart. This is the role played by the following
view~\cite{DBLP:journals/jfp/McBrideM04}. It is a sum type indexed by a thinning
and with one constructor for each smart constructor.
%
