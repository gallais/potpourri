...
In \idris{}, Haskell-style ad-hoc polymorphism is superseded by a more general
...
\subsection{Application: \IdrisType{Vect}, as \IdrisType{List}}
...
we will be able to break \IdrisBound{xs} in its constitutive parts and
...
\paragraph{Application: \IdrisFunction{map}}
...
definition to have it take an extra argument, here the result of the intermediate
computation (\IdrisFunction{view} \IdrisBound{xs}).
%
Correspondingly, we have locally extended the pattern-matching definition with
an additional pattern on the left-hand side of the indented with-clauses
(the pattern (\IdrisKeyword{\KatlaUnderscore} \IdrisKeyword{|}) signals to Idris
that we simply reproduce the parent pattern with no modification).
%
By matching on the result of the \IdrisFunction{view} call, we get to `match'
on \IdrisBound{xs} as if \IdrisType{Vect} were an inductive type. This is precisely
the power of views.

On the right-hand side the smart constructors let us build the output vector.

\paragraph{Application: \IdrisFunction{lookup}}
...
\todo{explain xs@\_}
\ExecuteMetaData[LookupExpanded.idr.tex]{lookup}
