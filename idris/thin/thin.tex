...
\begin{abstract}
  Remembering that types give us a way to interpret unstructured data stored in
  memory, we use Quantitative Type Theory as implemented in \idris{} to define an
  invariant-rich typechecking time representation that is guaranteed to compile
...
However relying on inductive families can have a runtime cost.
The prototypical example is probably the lookup function for vectors.
\todo{quadratic blowup}

There has already been extensive work on erasure to automatically detect redundant
data~\cite{DBLP:conf/types/BradyMM03} (e.g. the natural number a vector is indexed
by is entirely determined by the vector's spine) or data that will not be used at
