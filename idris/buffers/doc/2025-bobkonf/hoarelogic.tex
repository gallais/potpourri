...
\begin{frame}{What we {\bf want}: lifting results}
{\huge
\begin{mathpar}
 \inferrule{
...
\begin{frame}{What we {\bf get}: nonsense!}
...
\begin{tikzpicture}[tpstyle]
  \node<4->[pencil,ultra thick,draw, minimum width=.75cm, minimum height=1.75cm, yshift=.4cm, ellipse] (box1) at (node) {};
  \draw<4->[arrow,->] (box1.south) to[bend left] +(-2,-1) node[anchor=east]
       {\hand $0$ is equal to $1$?!};
